...
function goopterLoadPayPalScript(config, onLoaded, onError) {
...
function canShowPlaceOrderBtn() {
    ...
    if (goopterOrder.isPpcpPaymentMethodSelected() && goopter_ppcp_manager.is_hide_place_order_button === 'no') {
        return true;
    }
    return !(!isOrderCompletePage
        && goopterOrder.isGoopterPaymentMethodSelected()
        && !goopterOrder.isSavedPaymentMethodSelected());
}
