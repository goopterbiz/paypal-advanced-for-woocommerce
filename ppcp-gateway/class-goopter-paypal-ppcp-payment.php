...
class Goopter_PayPal_PPCP_Payment {
    ...
    public function getCartLineItems(): array {
        ...
        foreach (WC()->cart->get_fees() as $item) {
            $lineItems[] = [
                'label' => html_entity_decode(wc_trim_string($item->name ?: __('Fee', 'paypal-advanced-for-woocommerce'), 127), ENT_NOQUOTES, 'UTF-8'),
    ...
    public function goopter_ppcp_get_paypal_line_items_from_cart() {
        try {
            ...
            foreach (WC()->cart->get_fees() as $cart_item_key => $fee_values) {
                ...
                if ($amount > 0) {
                    $fee_item = array(
                        'name' => html_entity_decode(wc_trim_string($fee_values->name ? $fee_values->name : __('Fee', 'paypal-advanced-for-woocommerce'), 127), ENT_NOQUOTES, 'UTF-8'),
                        'description' => '',
                        'quantity' => 1,
                        'amount' => Goopter_Gateway_Paypal::number_format($fee_values->amount),
                    );
            ...
            if (isset($this->api_response['id']) && !empty($this->api_response['id'])) {
                ...
                if ($this->api_response['status'] == 'COMPLETED') {
                    ...
                    if (!empty($payment_source['card'])) {
                        $card_response_order_note = __('Card Details', 'paypal-advanced-for-woocommerce');
                    ...
                    foreach ($this->api_response['purchase_units'] as $captures_key => $captures) {
                        ...
                        if (!empty($processor_response['avs_code'])) {
                            $avs_response_order_note = __('Address Verification Result', 'paypal-advanced-for-woocommerce');
                        ...
                        if (!empty($processor_response['cvv_code'])) {
                            $cvv2_response_code = __('Card Security Code Result', 'paypal-advanced-for-woocommerce');
                            $cvv2_response_code .= "\n";
                        ...
                        if (!empty($processor_response['response_code'])) {
                            $response_code = __('Processor response code Result', 'paypal-advanced-for-woocommerce');
                            $response_code .= "\n";
                    ...
                    if (count($this->api_response['purchase_units']) === 1) {
                        if ($payment_status == 'COMPLETED') {
                            ...
                            $order->add_order_note(sprintf(__('Payment via %1$s: %2$s.', 'paypal-advanced-for-woocommerce'), $order->get_payment_method_title(), ucfirst(strtolower($payment_status))));
                        } else {
                    ...
                    if (!empty($processor_response['payment_advice_code'])) {
                        $payment_advice_code = __('Payment Advice Codes Result', 'paypal-advanced-for-woocommerce');
                        $payment_advice_code .= "\n";
                    ...
                    $order->add_order_note(sprintf(__('%1$s Capture Transaction ID: %2$s', 'paypal-advanced-for-woocommerce'), $goopter_ppcp_payment_method_title, $transaction_id));
                ...
                wc_add_notice(__('This payment was unable to be processed successfully. Please try again with another payment method.', 'paypal-advanced-for-woocommerce'), 'error');
    ...
    public function goopter_ppcp_get_paypal_line_items_from_order($order) {
        try {
            ...
            foreach ($order->get_fees() as $fee_values) {
                ...
                if ($amount > 0) {
                    $item = array(
                        'name' => html_entity_decode(wc_trim_string($fee_item_name ? $fee_item_name : __('Fee', 'paypal-advanced-for-woocommerce'), 127), ENT_NOQUOTES, 'UTF-8'),
                        'description' => '',
                        'quantity' => 1,
    ...
    public function goopter_ppcp_refund_order($order_id, $amount, $reason, $transaction_id) {
        try {
            ...
            if (isset($this->api_response['status']) && $this->api_response['status'] == "COMPLETED") {
                ...
                $order->add_order_note(sprintf(__('Payment via %1$s Pending. Pending reason: %2$s.', 'paypal-advanced-for-woocommerce'), $goopter_ppcp_payment_method_title, $pending_reason_text));
                $order->add_order_note(
                    // Translators: %1$s is the refunded amount with currency, %2$s is the refund transaction ID.
            ...
            if (!empty($this->api_response['id'])) {
                ...
                if ($this->api_response['status'] == 'COMPLETED' && strtolower($payment_status) != "denied") {
                    ...
                    if ($payment_status == 'COMPLETED') {
                        ...
                        $order->payment_complete($transaction_id);
                        // Translators: %1$s is the payment method title, %2$s is the payment status.
                        $order->add_order_note(sprintf(__('Payment via %1$s: %2$s.', 'paypal-advanced-for-woocommerce'), $goopter_ppcp_payment_method_title, ucfirst(strtolower($payment_status))));
                    } elseif ($payment_status === 'DECLINED') {
                        // Translators: %s is the payment method title.
                        $order->update_status('failed', sprintf(__('Payment via %s declined.', 'paypal-advanced-for-woocommerce'), $goopter_ppcp_payment_method_title));
                        wc_add_notice(__('Unfortunately your order cannot be processed as the originating bank/merchant has declined your transaction. Please attempt your purchase again.', 'paypal-advanced-for-woocommerce'), 'error');
                    ...
                    $order->add_order_note(sprintf(__('%1$s Authorization Transaction ID: %2$s', 'paypal-advanced-for-woocommerce'), $goopter_ppcp_payment_method_title, $transaction_id));
                    $order->add_order_note('Seller Protection Status: ' . goopter_ppcp_readable($seller_protection));
                    ...
                    if ($this->is_auto_capture_auth) {
                        $order->add_order_note(__('Payment authorized. Change payment status to processing or complete to capture funds.', 'paypal-advanced-for-woocommerce'));
                    }
                    $order->save();
                    return true;
                } else {
    ...
    public function goopter_ppcp_update_woo_order_data($paypal_order_id) {
        ...
        if ($this->paymentaction === 'capture' && !empty($this->checkout_details->status) && $this->checkout_details->status == 'COMPLETED' && $order !== false) {
            ...
            if (!empty($payment_status_reason)) {
                ...
                $order->add_order_note(sprintf(__('Payment via %1$s Pending. PayPal reason: %2$s.', 'paypal-advanced-for-woocommerce'), $goopter_ppcp_payment_method_title, $payment_status_reason));
            }
            $order->set_transaction_id($transaction_id);
            $order->update_meta_data('_payment_status', $payment_status);
            $order->update_meta_data('_auth_transaction_id', $transaction_id);
            ...
            // Translators: %1$s is the payment method title, %2$s is the authorization transaction ID.
            $order->add_order_note(sprintf(__('%1$s Authorization Transaction ID: %2$s', 'paypal-advanced-for-woocommerce'), $this->title, $transaction_id));
            $order->add_order_note('Seller Protection Status: ' . goopter_ppcp_readable($seller_protection));
            if (class_exists('Goopter_PayPal_PPCP_Admin_Action')) {
                Goopter_PayPal_PPCP_Admin_Action::instance()->removeAutoCaptureHooks();
            }
            ...
            $order->save();
            $order->add_order_note(__('Payment authorized. Change order status to processing or complete for capture funds.', 'paypal-advanced-for-woocommerce'));
        }
    }
        ...
        if (function_exists('WC')) {
            try {
                ...
                if (!empty($error_email_notification_param['request'])) {
                    $message .= "<strong>" . __('Action: ', 'paypal-advanced-for-woocommerce') . "</strong>" . ucwords(str_replace('_', ' ', $error_email_notification_param['request'])) . PHP_EOL;
                }
                if (!empty($error_message)) {
                    $message .= "<strong>" . __('Error: ', 'paypal-advanced-for-woocommerce') . "</strong>" . $error_message . PHP_EOL;
                }
                if (!empty($error_email_notification_param['order_id'])) {
                    $message .= "<strong>" . __('Order ID: ', 'paypal-advanced-for-woocommerce') . "</strong>" . $error_email_notification_param['order_id'] . PHP_EOL;
                }
                if (is_user_logged_in()) {
                    $userLogined = wp_get_current_user();
                    $message .= "<strong>" . __('User ID: ', 'paypal-advanced-for-woocommerce') . "</strong>" . $userLogined->ID . PHP_EOL;
                    $message .= "<strong>" . __('User Email: ', 'paypal-advanced-for-woocommerce') . "</strong>" . $userLogined->user_email . PHP_EOL;
                }
                ...
                case 'PENDING' :
                    switch (strtoupper($pending_reason)) {
                        case 'BUYER_COMPLAINT':
                            $pending_reason_text = __('BUYER_COMPLAINT: The payer initiated a dispute for this captured payment with PayPal.', 'paypal-advanced-for-woocommerce');
                            break;
                        case 'CHARGEBACK':
                            $pending_reason_text = __('CHARGEBACK: The captured funds were reversed in response to the payer disputing this captured payment with the issuer of the financial instrument used to pay for this captured payment.', 'paypal-advanced-for-woocommerce');
                            break;
                        case 'ECHECK':
                            $pending_reason_text = __('ECHECK: The payer paid by an eCheck that has not yet cleared.', 'paypal-advanced-for-woocommerce');
                            break;
                        case 'INTERNATIONAL_WITHDRAWAL':
                            $pending_reason_text = __('INTERNATIONAL_WITHDRAWAL: Visit your online account. In your **Account Overview**, accept and deny this payment.', 'paypal-advanced-for-woocommerce');
                            break;
                        case 'OTHER':
                            $pending_reason_text = __('No additional specific reason can be provided. For more information about this captured payment, visit your account online or contact PayPal.', 'paypal-advanced-for-woocommerce');
                            break;
                        case 'PENDING_REVIEW':
                            $pending_reason_text = __('PENDING_REVIEW: The captured payment is pending manual review.', 'paypal-advanced-for-woocommerce');
                            break;
                        case 'RECEIVING_PREFERENCE_MANDATES_MANUAL_ACTION':
                            $pending_reason_text = __('RECEIVING_PREFERENCE_MANDATES_MANUAL_ACTION: The payee has not yet set up appropriate receiving preferences for their account. For more information about how to accept or deny this payment, visit your account online. This reason is typically offered in scenarios such as when the currency of the captured payment is different from the primary holding currency of the payee.', 'paypal-advanced-for-woocommerce');
                            break;
                        case 'REFUNDED':
                            $pending_reason_text = __('REFUNDED: The captured funds were refunded.', 'paypal-advanced-for-woocommerce');
                            break;
                        case 'TRANSACTION_APPROVED_AWAITING_FUNDING':
                            $pending_reason_text = __('TRANSACTION_APPROVED_AWAITING_FUNDING: The payer must send the funds for this captured payment. This code generally appears for manual EFTs.', 'paypal-advanced-for-woocommerce');
                            break;
                        case 'UNILATERAL':
                            $pending_reason_text = __('UNILATERAL: The payee does not have a PayPal account.', 'paypal-advanced-for-woocommerce');
                            break;
                        case 'VERIFICATION_REQUIRED':
                            $pending_reason_text = __('VERIFICATION_REQUIRED: The payee\'s PayPal account is not verified.', 'paypal-advanced-for-woocommerce');
                            break;
                        case 'none':
                        default:
                            $pending_reason_text = __('No pending reason provided.', 'paypal-advanced-for-woocommerce');
                            break;
                    }
                    if ($payment_status === 'PENDING') {
                        // Translators: %1$s is the payment method title, %2$s is the PayPal pending reason text.
                        $order->update_status('on-hold', sprintf(__('Payment via %1$s Pending. PayPal Pending reason: %2$s.', 'paypal-advanced-for-woocommerce'), $goopter_ppcp_payment_method_title, $pending_reason_text));
                    }
                    if ($payment_status === 'DECLINED') {
                        // Translators: %1$s is the payment method title, %2$s is the PayPal declined reason text.
                        $order->update_status('failed', sprintf(__('Payment via %1$s declined. PayPal declined reason: %2$s.', 'paypal-advanced-for-woocommerce'), $goopter_ppcp_payment_method_title, $pending_reason_text));
                    }
                    break;
                case 'PARTIALLY_REFUNDED' :
                    $order->update_status('on-hold');
                    // Translators: %1$s is the payment method title, %2$s is the PayPal refund reason.
                    $order->add_order_note(sprintf(__('Payment via %1$s partially refunded. PayPal reason: %2$s.', 'paypal-advanced-for-woocommerce'), $goopter_ppcp_payment_method_title, $pending_reason));
                    break;
                case 'REFUNDED' :
                    $order->update_status('refunded');
                    // Translators: %1$s is the payment method title, %2$s is the PayPal refund reason.
                    $order->add_order_note(sprintf(__('Payment via %1$s refunded. PayPal reason: %2$s.', 'paypal-advanced-for-woocommerce'), $goopter_ppcp_payment_method_title, $pending_reason));
                    break;
                case 'FAILED' :
                    // Translators: %1$s is the payment method title, %2$s is the PayPal failure reason.
                    $order->update_status('failed', sprintf(__('Payment via %1$s failed. PayPal reason: %2$s.', 'paypal-advanced-for-woocommerce'), $goopter_ppcp_payment_method_title, $pending_reason));
                    break;
                case 'VOIDED' :
                    // Translators: %s is the payment method title.
        ...
        try {
            ...
            switch (strtoupper($payment_status)) :
                ...
                case 'PENDING' :
                    ...
                    if ($payment_status === 'PENDING') {
                        // Translators: %1$s is the payment method title, %2$s is the PayPal pending reason.
                        $order->add_order_note(sprintf(__('Payment via %1$s Pending. PayPal Pending reason: %2$s.', 'paypal-advanced-for-woocommerce'), $order->get_payment_method_title(), $pending_reason_text));
                    }
                    if ($payment_status === 'DECLINED') {
                        // Translators: %1$s is the payment method title, %2$s is the PayPal declined reason.
                        $order->add_order_note(sprintf(__('Payment via %1$s declined. PayPal declined reason: %2$s.', 'paypal-advanced-for-woocommerce'), $order->get_payment_method_title(), $pending_reason_text));
                    }
                    break;
                case 'PARTIALLY_REFUNDED' :
                    // Translators: %1$s is the payment method title, %2$s is the PayPal refund reason.
                    $order->add_order_note(sprintf(__('Payment via %1$s partially refunded. PayPal reason: %2$s.', 'paypal-advanced-for-woocommerce'), $order->get_payment_method_title(), $pending_reason));
                case 'REFUNDED' :
                    // Translators: %1$s is the payment method title, %2$s is the PayPal refund reason.
                    $order->add_order_note(sprintf(__('Payment via %1$s refunded. PayPal reason: %2$s.', 'paypal-advanced-for-woocommerce'), $order->get_payment_method_title(), $pending_reason));
                case 'FAILED' :
                    // Translators: %1$s is the payment method title, %2$s is the PayPal failure reason.
                    $order->add_order_note(sprintf(__('Payment via %1$s failed. PayPal reason: %2$s.', 'paypal-advanced-for-woocommerce'), $order->get_payment_method_title(), $pending_reason));
                    break;
                default:
                    break;
            endswitch;
    ...
    public function goopter_ppcp_void_authorized_payment_admin($order, $order_data) {
        try {
            ...
            if (!is_wp_error($response)) {
                ...
                $order->add_order_note(__("Void Authorization Failed:", 'paypal-advanced-for-woocommerce') . ': ' . $response->get_error_message());
            }
        } catch (Exception $ex) {
            $this->api_log->log("The exception was created on line: " . $ex->getFile() . ' ' . $ex->getLine(), 'error');
            $this->api_log->log($ex->getMessage(), 'error');
                ...
                if (!empty($this->api_response['id'])) {
                    ...
                    if (goopter_ppcp_get_token_id_by_token($this->api_response['id']) === '') {
                        ...
                        if ($token->validate()) {
                            ...
                            $order->add_order_note('ERROR MESSAGE: ' . __('Invalid or missing payment token fields.', 'paypal-advanced-for-woocommerce'));
                        }
                    } else {
                        add_filter('woocommerce_payment_complete_order_status', function ($payment_status, $order_id) {
                            return $this->get_preferred_order_status($payment_status, $order_id);
