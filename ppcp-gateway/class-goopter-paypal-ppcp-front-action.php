        ...
        try {
            ...
            if (!empty($goopter_ppcp_paypal_order_id)) {
                ...
                if ($liability_shift_result === 1) {
                    ...
                    $is_success = false;
                    wc_add_notice(__('We cannot process your order with the payment information that you provided. Please use an alternate payment method.', 'goopter-advanced-payment-for-paypal-complete-payment-and-woocommerce'), 'error');
                } elseif ($liability_shift_result === 3) {
                    $is_success = false;
                    wc_add_notice(__('Something went wrong. Please try again.', 'goopter-advanced-payment-for-paypal-complete-payment-and-woocommerce'), 'error');
                }
                if ($is_success) {
                    WC()->cart->empty_cart();
                    Goopter_Session_Manager::clear();
    ...
    public function goopter_ppcp_liability_shift($order, $response_object) {
        if (!empty($response_object)) {
            ...
            if (!empty($response['payment_source']['card']['authentication_result']['liability_shift'])) {
                ...
                $liability_shift_order_note = __('3D Secure response', 'goopter-advanced-payment-for-paypal-complete-payment-and-woocommerce');
