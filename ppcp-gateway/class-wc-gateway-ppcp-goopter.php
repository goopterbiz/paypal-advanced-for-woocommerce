...
class WC_Gateway_PPCP_Goopter extends WC_Payment_Gateway {
    ...
    public function __construct() {
        try {
            ...
            if (goopter_ppcp_has_active_session()) {
                $this->order_button_text = apply_filters('goopter_ppcp_order_review_page_place_order_button_text', __('Complete Order Payment', 'paypal-advanced-for-woocommerce'));
            }
    ...
    public function setup_properties() {
        ...
        $this->method_title = apply_filters('goopter_ppcp_gateway_method_title', sprintf('%s - Built by Goopter', AE_PPCP_NAME));
        $this->method_description = __('The easiest one-stop solution for accepting PayPal, Venmo, Debit/Credit Cards with cheaper fees than other processors!', 'paypal-advanced-for-woocommerce');
    }
    ...
    public function form() {
        ...
        $cvc_field = '<div class="form-row form-row-last">
                        <label for="' . esc_attr($this->id) . '-card-cvc">' . apply_filters('cc_form_label_card_code', __('Card Security Code', 'paypal-advanced-for-woocommerce'), $this->id) . ' </label>
        ...
        $default_fields = array(
            'card-number-field' => '<div class="form-row form-row-wide">
                        <label for="' . esc_attr($this->id) . '-card-number">' . apply_filters('cc_form_label_card_number', __('Card number', 'paypal-advanced-for-woocommerce'), $this->id) . '</label>
            ...
            'card-expiry-field' => '<div class="form-row form-row-first">
                        <label for="' . esc_attr($this->id) . '-card-expiry">' . apply_filters('cc_form_label_expiry', __('Expiration Date', 'paypal-advanced-for-woocommerce'), $this->id) . ' </label>
        ...
        if (apply_filters('goopter_is_ppcp_parallel_payment_not_used', true, $order_id)) {
            if($order && $this->can_refund_order($order) && goopter_ppcp_order_item_meta_key_exists($order, '_ppcp_capture_details')) {
                ...
                if (!$this->can_refund_order($order)) {
                    return new WP_Error('error', __('Refund failed.', 'paypal-advanced-for-woocommerce'));
                }
    ...
    public function goopter_ppcp_admin_notices() {
        ...
        $onboarding_success_message = sprintf(esc_html__('Your PayPal account has been connected successfully and you are ready to rock!', 'paypal-advanced-for-woocommerce'), wp_specialchars_decode(get_option('blogname'), ENT_QUOTES));
    ...
    public function save_payment_method_checkbox() {
        $html = sprintf(
                ...
                esc_attr($this->id),
                esc_html__('Save payment method to my account.', 'paypal-advanced-for-woocommerce')
        );
