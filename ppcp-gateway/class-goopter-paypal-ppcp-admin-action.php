...
class Goopter_PayPal_PPCP_Admin_Action {
        ...
        if (in_array($payment_method, ['goopter_ppcp_cc', 'goopter_ppcp', 'goopter_ppcp_apple_pay', 'goopter_ppcp_google_pay']) && $transaction_id && $paymentaction === 'authorize') {
            ...
            if ($this->goopter_ppcp_is_authorized_only($trans_details)) {
                ...
                if (!is_wp_error($response)) {
                    $note = __("Authorization Voided", 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment');
                } else {
                    $note = __("Void Authorization Failed:", 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment') . ': ' . $response->get_error_message();
                }
    ...
    public function admin_notices() {
        try {
            ...
            if (isset($_GET['page']) && 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment' === $_GET['page']) {
                return;
            }
            ...
            if (!empty($gt_ppcp_account_reconnect_notice) && !empty($notice_type['active_ppcp_gateways']) && isset($notice_type['active_ppcp_gateways']['goopter_ppcp'])) {
                ...
                $notice_data_account_reconnect = array(
                    ...
                    'ans_button_url' => admin_url('options-general.php?page=goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment'),
    ...
    public function goopter_ppcp_order_action_list($order_id) {
        try {
            $order = wc_get_order($order_id);
            if (!is_a($order, 'WC_Order')) {
                echo esc_html(__('Error: Unable to detect the order, please refresh again to retry or Contact PayPal For WooCommerce support.', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment'));
                return;
            }
            $this->order = $order;
            ...
            if (empty($paypal_order_id)) {
                //echo __('PayPal order id does not exist for this order.', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment');
                return;
            }
                ...
                if (isset($this->payment_response['purchase_units']['0']['payments']['authorizations']) && !empty($this->payment_response['purchase_units']['0']['payments']['authorizations'])) {
                    if (isset($this->payment_response['purchase_units']['0']['payments']['refunds'])) {
                        foreach ($this->payment_response['purchase_units']['0']['payments']['refunds'] as $key => $refunds) {
                            ...
                            $line_item['payment_action'] = __('Refund', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment');
                    ...
                    if (isset($this->payment_response['purchase_units']['0']['payments']['captures'])) {
                        foreach ($this->payment_response['purchase_units']['0']['payments']['captures'] as $key => $captures) {
                            ...
                            $line_item['payment_action'] = __('Capture', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment');
                    ...
                    if (isset($this->payment_response['purchase_units']['0']['payments']['authorizations']['0']['status']) && 'CREATED' === $this->payment_response['purchase_units']['0']['payments']['authorizations']['0']['status']) {
                        $this->goopter_ppcp_order_actions['void'] = __('Void Authorization', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment');
                        $this->goopter_ppcp_order_actions['capture'] = __('Capture Funds', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment');
                    }
                    if (isset($this->payment_response['purchase_units']['0']['payments']['authorizations']['0']['status']) && 'PARTIALLY_CAPTURED' === $this->payment_response['purchase_units']['0']['payments']['authorizations']['0']['status']) {
                        if ($this->gt_refund_amount < $this->gt_capture_amount) {
                            $this->goopter_ppcp_order_actions['refund'] = __('Refund', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment');
                            $this->goopter_ppcp_order_actions['void'] = __('Void Authorization', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment');
                            $this->goopter_ppcp_order_actions['capture'] = __('Capture Funds', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment');
                        }
    ...
    public function goopter_ppcp_order_meta_auth_capture_html() {
        ...
        <?php if (isset($this->goopter_ppcp_order_status_data['capture']) && isset($this->goopter_ppcp_order_actions['capture'])) { ?>
            <tr class="goopter_ppcp_capture_box" style="display: none;">
                <td class="label"><?php echo esc_html(__('Additional Capture Possible', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment')); ?></td>
                <td width="1%"></td>
                <td class="total">
                    <fieldset>
                        <label for="additional_capture_yes">
                            ...
                            Yes
                            <?php echo wp_kses_post(wc_help_tip(__('Yes (option to capture additional funds on this authorization if need)', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment'))); ?>
                        </label>
                        <label for="additional_capture_no">
                            <input type="radio" name="additionalCapture" value="no" id="additional_capture_no">
                            No
                            <?php echo wp_kses_post(wc_help_tip(__('No (no additional capture needed; close authorization after this capture)', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment'))); ?>
                        </label>
                    </fieldset>
                </td>
            </tr>
            <tr class="goopter_ppcp_capture_box" style="display: none;">
                <td class="label">
                    <label for="refund_amount">
                        ...
                        <?php esc_html_e('Amount', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment'); ?>:
                    </label>
                </td>
                <td width="1%"></td>
                <td class="total">
        ...
        <?php if (isset($this->goopter_ppcp_order_status_data['refund']) && isset($this->goopter_ppcp_order_actions['refund'])) { ?>
            <tr class="goopter_ppcp_refund_box" style="display: none;">
                <td class="label"><?php echo esc_html(__('Transaction Id', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment')); ?></td>
                <td width="1%"></td>
                <td class="total">
                    <select name="goopter_ppcp_refund_data" id="goopter_ppcp_refund_data" style="width: 250px;">
                        ...
                        foreach ($this->goopter_ppcp_order_status_data['refund'] as $k => $v) :
                            if ($i == 0) {
                                echo "<option value=''>" . esc_html(__('Select Transaction Id', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment')) . "</option>";
                            }
                            ?>
            ...
            <tr class="goopter_ppcp_refund_box" style="display: none;">
                <td class="label"><?php echo esc_html(__('Refund Amount', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment')); ?></td>
                <td width="1%"></td>
                <td class="total">
                    <fieldset>
            ...
            foreach ($ppcp_capture_details as $index => $meta_array) :
                ...
                <tr>
                    <td>
                        ...
                        echo '<b>' . esc_html(__('Capture Details', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment')) . '</b>: ';
            ...
            foreach ($ppcp_refund_details as $index => $meta_array) :
                ...
                <tr>
                    <td>
                        ...
                        echo '<b>' . esc_html(__('Refund Details', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment')) . '</b>: ';
                        $refund_details_html = '';
                        if (is_array($meta_array) && !empty($meta_array)) {
    ...
    public function goopter_ppcp_display_total_capture($order_id) {
        ...
        <tr>
            <td class="label">
                <?php esc_html_e('Total Capture:', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment'); ?>
            </td>
            <td width="1%"></td>
            <td class="total">
    ...
    public function goopter_ppcp_display_payment_authorization_notice() {
        ...
        if ($screen && $this->goopter_ppcp_is_display_paypal_transaction_details($order->get_id())) {
            echo wp_kses_post('<div class="updated woocommerce-message"><p>' . esc_html__('Capture the authorized order to receive funds in your PayPal account.', 'goopter-advanced-payment-for-woocommerce-and-paypal-complete-payment') . '</p></div>');
        }
    }
