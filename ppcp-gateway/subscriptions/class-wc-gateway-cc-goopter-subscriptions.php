...
class Goopter_WC_Gateway_CC_Subscriptions extends Goopter_WC_Gateway_CC {

    public function __construct() {
        parent::__construct();
        if (class_exists('WC_Subscriptions_Order')) {
            if (!has_action('woocommerce_scheduled_subscription_payment_goopter_ppcp_cc', array($this, 'scheduled_subscription_payment'))) {
                add_action('woocommerce_scheduled_subscription_payment_goopter_ppcp_cc', array($this, 'scheduled_subscription_payment'), 10, 2);
            }
            ...
            add_action('woocommerce_subscription_failing_payment_method_updated_goopter_ppcp_cc', array($this, 'update_failing_payment_method'), 10, 2);
        }
    }
    ...
    public function process_payment($order_id) {
        if ($this->is_subscription($order_id)) {
            if (goopter_ppcp_is_subs_change_payment()) {
            ...
            } elseif ($this->goopter_ppcp_is_free_signup_with_free_trial() === true) {
                return parent::goopter_ppcp_process_free_signup_with_free_trial($order_id);
            } else {
    ...
    public function goopter_ppcp_is_free_signup_with_free_trial() {
    ...
    public function scheduled_subscription_payment($amount_to_charge, $renewal_order) {
        ...
        $payment_tokens_id = goopter_ppcp_get_post_meta($renewal_order, '_payment_tokens_id', true);
        if (empty($payment_tokens_id) || $payment_tokens_id == false) {
            $this->goopter_scheduled_subscription_payment_retry_compability($renewal_order);
        }
        ...
        if (!empty($subscriptions)) {
            foreach ($subscriptions as $subscription) {
                ...
                if (!empty($goopter_ppcp_used_payment_method)) {
                    $renewal_order->update_meta_data('_goopter_ppcp_used_payment_method', $goopter_ppcp_used_payment_method);
                    $renewal_order->save();
                }
            }
        }
    ...
    public function goopter_scheduled_subscription_payment_retry_compability($renewal_order) {
        $payment_tokens_id = goopter_ppcp_get_post_meta($renewal_order, '_payment_tokens_id', true);
        if (empty($payment_tokens_id) || $payment_tokens_id == false) {
            if (function_exists('wcs_order_contains_subscription') && wcs_order_contains_subscription($renewal_order->get_id())) {
                $subscriptions = wcs_get_subscriptions_for_order($renewal_order->get_id());
            ...
            if (!empty($subscriptions)) {
                foreach ($subscriptions as $subscription) {
                    ...
                    if (is_a($subscription_parent, WC_Subscription::class)) {
                        ...
                        $goopter_ppcp_used_payment_method = $subscription_parent->get_meta('_goopter_ppcp_used_payment_method', true);
                        if (!empty($goopter_ppcp_used_payment_method)) {
                            $renewal_order->update_meta_data('_goopter_ppcp_used_payment_method', $goopter_ppcp_used_payment_method);
