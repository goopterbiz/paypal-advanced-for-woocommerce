...
class Goopter_PayPal_PPCP_Product extends WC_Form_Handler {
        ...
        try {
            ...
            if (!empty($items)) {
                ...
                if (!$was_added_to_cart && !$quantity_set) {
                    wc_add_notice(__('Please choose the quantity of items you wish to add to your cart&hellip;', 'goopter-advanced-payment-for-paypal-complete-payment-and-woocommerce'), 'error');
            ...
            // phpcs:enable WordPress.Security.NonceVerification.Recommended -- no security issue
                ...
                wc_add_notice(__('Please choose a product to add to your cart&hellip;', 'goopter-advanced-payment-for-paypal-complete-payment-and-woocommerce'), 'error');
            }
            return false;
        } catch (Exception $ex) {
            self::$api_log->log("The exception was created on line: " . $ex->getFile() . ' ' .$ex->getLine(), 'error');
    ...
    private static function goopter_ppcp_add_to_cart_handler_variable($product_id) {
        try {
            ...
            if (empty($variation_id)) {
                throw new Exception(__('Please choose product options&hellip;', 'goopter-advanced-payment-for-paypal-complete-payment-and-woocommerce'));
            }
            ...
            foreach ($adding_to_cart->get_attributes() as $attribute) {
                ...
                if (isset($posted_attributes[$attribute_key])) {
                    ...
                    if ($valid_value === $value) {
                        ...
                        throw new Exception(sprintf(__('Invalid value posted for %s', 'goopter-advanced-payment-for-paypal-complete-payment-and-woocommerce'), wc_attribute_label($attribute['name'])));
                    }
                } elseif ('' === $valid_value) {
            ...
            if (!empty($missing_attributes)) {
                ...
                throw new Exception(sprintf(_n('%s is a required field', '%s are required fields', count($missing_attributes), 'goopter-advanced-payment-for-paypal-complete-payment-and-woocommerce'), wc_format_list_of_items($missing_attributes)));
            }
        } catch (Exception $e) {
