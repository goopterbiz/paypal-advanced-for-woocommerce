...
class Goopter_PayPal_PPCP_Product extends WC_Form_Handler {
        ...
        try {
            ...
            if (!empty($items)) {
                ...
                if (!$was_added_to_cart && !$quantity_set) {
                    wc_add_notice(__('Please choose the quantity of items you wish to add to your cart&hellip;', 'paypal-advanced-for-woocommerce'), 'error');
                ...
                wc_add_notice(__('Please choose a product to add to your cart&hellip;', 'paypal-advanced-for-woocommerce'), 'error');
            }
            return false;
        } catch (Exception $ex) {
            self::$api_log->log("The exception was created on line: " . $ex->getFile() . ' ' .$ex->getLine(), 'error');
    ...
    private static function goopter_ppcp_add_to_cart_handler_variable($product_id) {
        try {
            ...
            if (empty($variation_id)) {
                throw new Exception(__('Please choose product options&hellip;', 'paypal-advanced-for-woocommerce'));
            }
