...
<tr class="item <?php echo esc_attr( $row_class ); ?>" data-order_item_id="<?php echo esc_attr( $item_id ); ?>">
	...
	<td class="name" data-sort-value="<?php echo esc_attr( $item->get_name() ); ?>">
		...
		if ( $product && $product->get_sku() ) {
			echo '<div class="wc-order-item-sku"><strong>' . esc_html__( 'SKU:', 'goopter-advanced-integration-for-paypal-complete-payments-and-for-woocommerce' ) . '</strong> ' . esc_html( $product->get_sku() ) . '</div>';
		}

		if ( $item->get_variation_id() ) {
			echo '<div class="wc-order-item-variation"><strong>' . esc_html__( 'Variation ID:', 'goopter-advanced-integration-for-paypal-complete-payments-and-for-woocommerce' ) . '</strong> ';
			if ( 'product_variation' === get_post_type( $item->get_variation_id() ) ) {
				echo esc_html( $item->get_variation_id() );
			} else {
				/* translators: %s: variation id */
				printf( esc_html__( '%s (No longer exists)', 'goopter-advanced-integration-for-paypal-complete-payments-and-for-woocommerce' ), esc_html( $item->get_variation_id() ) );
			}
			echo '</div>';
		}
		?>
	...
	<td class="line_cost" width="1%" data-sort-value="<?php echo esc_attr( $item->get_total() ); ?>">
		<div class="view">
			...
			if ( $item->get_subtotal() !== $item->get_total() ) {
				...
				echo '<span class="wc-order-item-discount">' . sprintf( esc_html__( '%s discount', 'goopter-advanced-integration-for-paypal-complete-payments-and-for-woocommerce' ), wc_price( wc_format_decimal( $item->get_subtotal() - $item->get_total(), '' ), array( 'currency' => $order->get_currency() ) ) ) . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		...
		<div class="edit" style="display: none;">
			<div class="split-input">
				<div class="input">
					<label><?php esc_attr_e( 'Before discount', 'goopter-advanced-integration-for-paypal-complete-payments-and-for-woocommerce' ); ?></label>
				...
				<div class="input">
					<label><?php esc_attr_e( 'Total', 'goopter-advanced-integration-for-paypal-complete-payments-and-for-woocommerce' ); ?></label>
					<input type="text" name="line_total[<?php echo absint( $item_id ); ?>]" placeholder="<?php echo esc_attr( wc_format_localized_price( 0 ) ); ?>" value="<?php echo esc_attr( wc_format_localized_price( $item->get_total() ) ); ?>" class="line_total wc_input_price" data-tip="<?php esc_attr_e( 'After pre-tax discounts.', 'goopter-advanced-integration-for-paypal-complete-payments-and-for-woocommerce' ); ?>" data-total="<?php echo esc_attr( wc_format_localized_price( $item->get_total() ) ); ?>" />
				</div>
			</div>
		</div>
		<div class="refund" style="display: none;">
	...
	<td class="wc-order-edit-line-item" width="1%">
		<div class="wc-order-edit-line-item-actions">
			<?php if ( $order->is_editable() ) : ?>
				<a class="edit-order-item tips" href="#" data-tip="<?php esc_attr_e( 'Edit item', 'goopter-advanced-integration-for-paypal-complete-payments-and-for-woocommerce' ); ?>"></a><a class="delete-order-item tips" href="#" data-tip="<?php esc_attr_e( 'Delete item', 'goopter-advanced-integration-for-paypal-complete-payments-and-for-woocommerce' ); ?>"></a>
			<?php endif; ?>
		</div>
	</td>
</tr>
